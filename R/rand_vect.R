#' Random vector of N integers that sum to M
#'
#' @description Random vector of N integers that sum to M (https://stackoverflow.com/questions/24845909/generate-n-random-integers-that-sum-to-m-in-r)
#' @name rand_vect
#' @usage rand_vect(N,M)
#' @param N Size of the vector.
#' @param M Sum of elements in the vector.
#' @param sd Approximate standard deviation for elements of vector (default = 1).
#' @param pos.only Resulting vector contains only positive integers (default = TRUE)
#' @return Vector of size \code{N} with elements summing to \code{M}
#' @examples
#' rand_vect(3, 50)
#' [1] 17 16 17
